...
CSV_PATH = (HERE / TICKER_FILE).resolve()
...
IS_VALE3 = os.path.basename(TICKER_FILE) == "df_vale3_cleansing.csv"

df = pd.read_csv(CSV_PATH, encoding='utf8', delimiter=',', parse_dates=True, index_col=0).sort_index()
...
df_norm = (df - train_min) / train_rng
...
def create_dataset_multi(df_norm, target_col='preco', seq_len=21, pred_len=5):
    Xs, ys = [], []
...
X_all, y_all = create_dataset_multi(df_norm, 'preco', SEQ_LEN, PRED_LEN)
split_idx = TRAIN_LEN - SEQ_LEN
...
te_dl = DataLoader(DS(X_test, y_test), batch_size=512, shuffle=False, drop_last=False)
...
truth_index = df.index[TRAIN_LEN : TRAIN_LEN + len(y_pred_den)]
...
y_pred_np = y_pred_series.values
y_true_np = y_true_series.values
...
OUTDIR = (HERE / 'resultadosMetricas_MaxTrain' / TICKER_TAG)
...
fig2_path = OUTDIR / f'patchtst_pred_{TICKER_TAG}_max_train_fixed_test.png'
...
ax.set_title(
    f"MAPE={mape:.2f}% | MSE={mse:.2f} | RMSE={rmse:.2f} | MASE={mase:.3f} | BIAS={bias:.2f}",
    fontsize=22, pad=16
)
...
csv_pred_path = OUTDIR / f'predicoes_pred_{TICKER_TAG}_max_train_fixed_test.csv'
...
with open(OUTDIR / 'metrics.txt', 'w', encoding='utf-8') as f:
    ...
    f.write(f"Bias (ME): {bias:.6f}\n")
    f.write(f"Best Val Loss: {best_val:.6f}\n")

print(f"\nPredição salva em: {OUTDIR}")
print(f"MAPE={mape:.4f}% | MSE={mse:.6f} | RMSE={rmse:.6f} | MASE={mase:.6f} | BIAS={bias:.6f}")